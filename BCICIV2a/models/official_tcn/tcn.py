"""Core TCN building blocks adapted from the official pytorch-tcn implementation."""
...
from .conv import TemporalConv1d
...
# Supported initializers are stored in a lookup table so string configs can be
# validated and instantiated lazily.
kernel_init_fn = dict(
...
class BaseTCN(nn.Module):
    """Shared helpers for full TCN modules and individual residual blocks."""
...
class TemporalBlock(BaseTCN):
    """Two-layer residual temporal block with optional conditioning and gating."""
    ...
    def __init__(
        ...
        # Match the residual path to the block output width when channels change.
    ...
    def apply_embeddings(
        ...
        for embedding, expected_shape in zip( embeddings, self.embedding_shapes ):
            ...
            if len( embedding.shape ) == 2:
                # Sample-wise embeddings are broadcast across the temporal axis.
                e.append( embedding.unsqueeze(2).repeat(1, 1, x.shape[2]) )
            elif len( embedding.shape ) == 3:
                # Sequence embeddings must already be aligned with the feature map.
        ...
        if self.embedding_mode == 'concat':
            # Concatenation keeps both sources explicit, then projects back.
            x = self.embedding_projection_2(
                torch.cat( [ x, e ], dim=1 )
                )
        elif self.embedding_mode == 'add':
            # Addition treats the embedding as a residual bias on the feature map.
            x = x + e
    ...
    def forward(
        ...
        # First temporal convolution expands the receptive field for this block.
        ...
        # Return both the residual output and the pre-residual branch output so
        # the top-level TCN can optionally aggregate skip connections.
        return self.activation_final(out + res), out



class TCN(BaseTCN):
    """Stack of dilated TemporalBlocks with optional skip/output projections."""
    ...
    def __init__(
        ...
        if dilations is None:
            if dilation_reset is None:
                ...
                # Repeat dilation growth after the requested receptive-field budget.
        ...
        if use_skip_connections:
            ...
            for i in range( len( num_channels ) ):
                # Align each skip branch to the final channel width before summing.
    ...
    def forward(
        ...
        if self.use_skip_connections:
            skip_connections = []
            # Collect intermediate block outputs and fuse them at the end.
