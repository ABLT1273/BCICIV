"""Padding helpers for causal and non-causal temporal convolutions."""
...
class TemporalPad1d(nn.Module):
    """Pad 1D temporal inputs and optionally maintain a causal streaming buffer."""
    ...
    def __init__(
        ...
        if causal:
            # Causal convolutions can only see the past, so pad on the left.
            self.left_padding = self.pad_len
            self.right_padding = 0
        else:
            # Non-causal convolutions distribute padding across both sides.
        ...
        # The buffer caches the most recent context window for blockwise inference.
        if buffer is None:
    ...
    def pad_inference(
        ...
        if buffer_io is None:
            ...
            if in_buffer is None:
                # If the caller did not provide an external buffer, expose the
                # module's internal state so it can be wired up on later steps.
                in_buffer = self.buffer
                buffer_io.append_internal_buffer( in_buffer )

        # Concatenate cached context with the new chunk before convolution.
        x = torch.cat(
            (in_buffer, x),
            -1,
            )
        ...
        if buffer_io is None:
            ...
            # External callers receive the updated state explicitly.
    ...
    def reset_buffer(self):
        """Reset the causal context buffer to zeros."""
        self.buffer.zero_()
