"""Temporal convolution wrappers that handle causal padding and streaming state."""
...
class TemporalConv1d(nn.Conv1d):
    """Conv1d variant with internally managed temporal padding."""
    ...
    def __init__(
        ...
        # The effective receptive field determines how much past context is needed.
    ...
    def forward(
        ...
        # Pad first, then delegate the actual convolution to nn.Conv1d.
...
class TemporalConvTranspose1d(nn.ConvTranspose1d):
    """ConvTranspose1d variant aligned with the streaming/cropping rules of pytorch-tcn."""
    ...
    def forward(
        ...
        if self.causal:
            x = self.padder(x, inference=inference, buffer_io=buffer_io)
            x = super().forward(x)
            # Remove the overlap introduced by transposed-convolution upsampling.
...
class DepthwiseConv1d( TemporalConv1d ):
    """TemporalConv1d with one convolution group per input channel."""
...
class PointwiseConv1d( nn.Conv1d ):
    """Lightweight 1x1 projection used for channel mixing."""
