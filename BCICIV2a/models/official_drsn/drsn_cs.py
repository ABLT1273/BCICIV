"""Official DRSN-CS style 1D residual shrinkage network used for EEG baselines."""
...
class BasicBlock(nn.Module):
    """Residual block with channel-wise soft-threshold shrinkage."""

    expansion = 1

    def __init__(self, in_channels, out_channels, stride=1):
        super().__init__()
        # Apply adaptive shrinkage after the second convolution to suppress
        # low-magnitude responses before adding the shortcut path.
        ...
        # Identity shortcut unless spatial/channel size changes.
        self.shortcut = nn.Sequential()

        # Use a 1x1 projection when residual/shortcut shapes differ.
    ...
    def forward(self, x):
        # Fuse residual and shortcut, then apply the block's final non-linearity.
...
class Shrinkage(nn.Module):
    """Learn an adaptive soft-threshold for each channel."""
    ...
    def forward(self, x):
        x_raw = x
        # Thresholding is applied on magnitudes, then the original sign is restored.
        x = torch.abs(x)
        x_abs = x
        x = self.gap(x)
        x = torch.flatten(x, 1)
        ...
        # average = x    #CW
        x = self.fc(x)
        # Convert the normalized gating output into an absolute threshold.
        x = torch.mul(average, x)
        x = x.unsqueeze(2)
        # Soft thresholding: keep only the part above threshold.
        sub = x_abs - x
        zeros = sub - sub
...
class RSNet(nn.Module):
    ...
    def forward(self, x):
        # Stem + four residual stages progressively expand channels and shrink time.
        output = self.conv1(x)
        output = self.conv2_x(output)
        output = self.conv3_x(output)
        output = self.conv4_x(output)
        output = self.conv5_x(output)
        # Global pooling converts the temporal feature map into a sample embedding.
...
def rsnet34():
    """Build the 34-layer RSNet variant."""
